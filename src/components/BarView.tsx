...
import { GlassWater, Check, Play, Hourglass, CheckSquare } from 'lucide-react';
...
export const BarView: React.FC<BarViewProps> = ({ state, lang }) => {
  ...
  // Filter orders that contain bar items that are active (not completely served)
  const activeBarTickets = state.orders
    .map((order) => {
      const barItems = order.items.filter(
        (item) => item.department === 'bar' && item.status !== 'served' && item.status !== 'cancelled'
      );
      return {
        ...order,
        items: barItems,
      ...
      <div className="max-w-7xl mx-auto flex flex-col gap-6">
        ...
        <div className="flex items-center justify-between border-b border-slate-800 pb-5">
          <div className="flex items-center gap-3">
            <div className="h-10 w-10 bg-blue-600 rounded-xl flex items-center justify-center text-white font-bold text-xl shadow-md">
              🍹
            </div>
            <div>
              <h1 className="text-xl font-bold tracking-tight text-white flex items-center gap-2">
                {t.barDashboard}
                <span className="text-xs font-semibold px-2 py-0.5 bg-blue-500/20 text-blue-400 rounded-md border border-blue-500/20">
                  {t.drinksOnly}
                </span>
              </h1>
              <p className="text-xs text-slate-400 mt-0.5">Tactile bar monitor for beverage-making and cocktail shaking queues.</p>
            </div>
          </div>

          <div className="text-xs font-semibold text-slate-400 bg-slate-800/60 px-3 py-1.5 rounded-lg border border-slate-700/50">
            {activeBarTickets.length} ACTIVE ORDERS
          </div>
        </div>

        {/* TICKETS CONTAINER GRID */}
        {activeBarTickets.length === 0 ? (
          <div className="bg-slate-800/40 border border-slate-800 rounded-2xl p-16 text-center text-slate-500 flex flex-col items-center justify-center min-h-[400px]">
            <GlassWater className="h-14 w-14 stroke-1 mb-4 text-slate-700" />
            <h3 className="font-bold text-slate-300 text-lg">{t.noTasks}</h3>
            <p className="text-xs text-slate-500 mt-1 max-w-sm">When beverage orders are placed, they immediately load here as tactile beverage cards.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {activeBarTickets.map((ticket) => {
                      ...
                      return (
                        <div key={item.id} className="pb-3.5 border-b border-slate-700/20 last:border-b-0 last:pb-0 flex flex-col justify-between gap-3">
                          <div className="flex items-start gap-3">
                            <span className="h-6 min-w-[24px] px-1.5 bg-blue-600/10 border border-blue-500/15 text-blue-400 font-extrabold text-sm rounded-md flex items-center justify-center">
                              {item.quantity}
                            </span>
